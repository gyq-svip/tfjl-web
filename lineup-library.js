...
(function () {
    ...
    function _featuredSync(item,pj){ const slug=_featSlug(item); ['A','B'].forEach(function(ab){ const id=slug+'_'+ab; const o=_load(); o['featured']=o['featured']||{}; o['featured'][id]=o['featured'][id]||{}; const slot=o['featured'][id]; const prefix=(ab==='A')?'my_':'teammate_'; const cards=(ab==='A'?pj.myHandCards:pj.teammateHandCards)||[]; const idToName={}; cards.forEach(function(c){ if(c&&c.id) idToName[c.id]=(c.name||c); }); const rawSkins=pj.cardSkins||{}; if(!slot.skin){ slot.skin={}; Object.keys(rawSkins).forEach(function(k){ if(k.indexOf(prefix)!==0) return; const nm=idToName[k]; const sv=rawSkins[k]; if(nm&&sv) slot.skin[nm]=sv; }); } const ch=(ab==='A'?pj.myChariot:pj.teammateChariot); if(!slot.cart && ch && ch.main) slot.cart=String(ch.main); if(!slot.cart2 && ch && ch.sub) slot.cart2=String(ch.sub); _save(o); }); }
