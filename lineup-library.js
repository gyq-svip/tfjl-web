...
(function () {
    ...
    window.openLineupLibraryPanel = function () {
        ...
        ov.addEventListener('contextmenu', function (e) {
            const sl = e.target.closest('.ll-slot');
            if (sl) { e.preventDefault(); window._llSkinMenu(sl); }
        });
    ...
    function _slotBadge(el, lv, mo, dr) {
    ...
    function _applySlots(root) {
        ...
        (function step() {
            ...
            Promise.resolve().then(function () { return window.applySkinBgToSlot(el, hero, hero, 'my', force); })
                .catch(function () {}).then(function () { _slotBadge(el, lv, mo, dr); step(); });
        })();
    }
    // 皮肤/融合菜单（点卡槽弹出）
    window._llSkinMenu = function (el) {
        ...
        h += '<div style="font-size:0.68rem;color:rgba(255,255,255,0.45);margin:6px 0 2px;">等级 / 魔化</div>';
        ...
        m.querySelector('#llLvInput').addEventListener('input', function () {
            ...
            const L1 = _slot(tab, lid);
            _slotBadge(el, (L1.lv && L1.lv[hero]) || '', (L1.mohua && L1.mohua[hero]) || '', _cardDr(_tableFor(tab, el.getAttribute('data-w')), hero));
        });
        m.querySelector('#llMoInput').addEventListener('change', function () {
            window._llSet(tab, lid, 'mohua', hero, this.checked ? '1' : '');
            const L2 = _slot(tab, lid);
            _slotBadge(el, (L2.lv && L2.lv[hero]) || '', (L2.mohua && L2.mohua[hero]) || '', _cardDr(_tableFor(tab, el.getAttribute('data-w')), hero));
        });
        setTimeout(function () {
    ...
    function _actCard(a) {
        ...
        h += '<div style="min-width:52px;padding-top:10px;"><b style="color:#ffd700;font-size:0.85rem;">第' + a.day + '天</b></div>';
        ['A', 'B'].forEach(function (ab) {
            const id = 'd' + a.day + ab;
            const L = _slot('activity', id);
            const table = _tableFor('act', ab.toLowerCase());
            const sum = _sumDr(table, a[ab]);
                h += '<div style="display:flex;align-items:center;gap:4px;margin-bottom:3px;flex-wrap:wrap;">';
                h += '<span style="font-weight:800;color:' + (ab === 'A' ? '#4ecdc4' : '#f0932b') + ';font-size:0.76rem;">' + ab + '</span>';
