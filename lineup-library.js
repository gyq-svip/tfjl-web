...
(function () {
    ...
    /* ===== 精选阵容：管理员从 projects 勾选添加，按 category 分区展示（复用活动阵容渲染） ===== */
    function _isAdmin(){ try{ if(typeof window.isAdmin==='function') return window.isAdmin(); }catch(e){} try{ return !!(window.getGistToken && window.getGistToken()); }catch(e){ return false; } }
    ...
    async function _featuredSave(items){ const tok=_llTok(); if(!tok) return false; try{ const r=await fetch('https://api.github.com/gists/'+_LL_IDX_GIST,{method:'PATCH',headers:{'Accept':'application/vnd.github.v3+json','Content-Type':'application/json','Authorization':'token '+tok},body:JSON.stringify({files:{'featured_lineups.json':{content:JSON.stringify({items:items},null,2)}}})}); return r.ok; }catch(e){ return false; } }
    function _featuredSync(file,pj){ const slug=String(file).replace('.json',''); ['A','B'].forEach(function(ab){ const id='feat_'+slug+'_'+ab; const o=_load(); o['featured']=o['featured']||{}; o['featured'][id]=o['featured'][id]||{}; const slot=o['featured'][id]; const skins=(ab==='A'?(pj.cardSkins||{}):(pj.teammateCardSkins||pj.cardSkins||{})); if(!slot.skin) slot.skin=JSON.parse(JSON.stringify(skins||{})); const ch=(ab==='A'?(pj.myChariot||{}):(pj.teammateChariot||{})); if(!slot.cart && ch.main) slot.cart=String(ch.main); if(!slot.cart2 && ch.sub) slot.cart2=String(ch.sub); _save(o); }); }
    function _featuredCard(file,pj){ const slug=String(file).replace('.json',''); const aH=(pj.myHandCards||[]).map(function(c){return c.name||c;}); const bH=(pj.teammateHandCards||[]).map(function(c){return c.name||c;}); let h='<div class="ll-card" style="border:1px solid rgba(255,215,0,0.2);border-radius:10px;padding:6px 9px;margin-bottom:10px;background:rgba(0,0,0,0.22);">'; h+='<div style="display:flex;align-items:flex-start;gap:14px;">'; ['A','B'].forEach(function(ab){ const id='feat_'+slug+'_'+ab; const L=_slot('featured',id); const st=(ab==='A')?{side:'my',table:'我的'}:{side:'teammate',table:'队友'}; h+='<div style="border-radius:9px;padding:4px 7px 7px;background:'+(ab==='A'?'rgba(78,205,196,0.07)':'rgba(240,147,43,0.07)')+';border:1px solid '+(ab==='A'?'rgba(78,205,196,0.32)':'rgba(240,147,43,0.32)')+';">'; h+='<div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;margin-bottom:4px;">'; if(ab==='A') h+='<b style="color:#ffd700;font-size:0.82rem;margin-right:4px;white-space:nowrap;">'+(pj.name||file)+'</b>'; h+='<span style="font-weight:800;color:'+(ab==='A'?'#4ecdc4':'#f0932b')+';font-size:0.76rem;">'+ab+'组</span>'; h+='<span style="color:rgba(255,255,255,0.45);font-size:0.62rem;">主</span>'+_cartSelect('featured',id,'cart',L.cart); h+='<span style="color:rgba(255,255,255,0.45);font-size:0.62rem;">副</span>'+_cartSelect('featured',id,'cart2',L.cart2); h+='<span class="ll-drsum" data-tab="featured" data-lid="feat_'+slug+'_'+ab+'" data-side="'+st.side+'" data-table="'+st.table+'" data-heroes="'+_esc(JSON.stringify(ab==='A'?aH:bH))+'" title="减伤明细" style="color:#ff8a80;font-size:0.64rem;font-weight:700;cursor:help;">减伤</span>'; if(ab==='A') h+='<button onclick="_llReset(&quot;featured&quot;,&quot;feat_&quot;+slug+&quot;_A&quot;)" title="重置该阵容个人设置" style="padding:2px 6px;border-radius:6px;border:1px solid rgba(255,107,107,0.45);background:rgba(255,107,107,0.14);color:#ff6b6b;cursor:pointer;font-size:0.62rem;font-weight:700;white-space:nowrap;">重置</button>'; h+='</div>'; h+='<div style="display:grid;grid-template-columns:repeat(5,94px);gap:6px;">'; (ab==='A'?aH:bH).forEach(function(n,i){ h+=_slotHtml(ab.toLowerCase(),i,n,'featured','feat_'+slug+'_'+ab,94); }); h+='</div></div>'; }); const sc=_dayScripts('feat_'+slug); h+='<div style="align-self:flex-start;display:flex;flex-direction:column;gap:5px;min-width:108px;">'; h+='<button id="llScriptBtn-feat_'+slug+'" onclick="_llScriptDlg(&quot;feat_&quot;+slug)" title="管理该阵容脚本" style="padding:5px 10px;border-radius:7px;border:1px solid rgba(240,147,43,0.45);background:rgba(240,147,43,0.12);color:#f0932b;font-size:0.74rem;font-weight:700;cursor:pointer;text-align:left;">阵容脚本（'+sc.length+'）</button>'; h+='<div id="llScripts-feat_'+slug+'">'+(typeof _scriptsHtml==='function'?_scriptsHtml(sc,'feat_'+slug):'')+'</div>'; h+='</div>'; h+='</div></div>'; return h; }
    async function _renderFeatured(){ const p=document.getElementById('llList'); if(!p) return; p.innerHTML='<div style="color:rgba(255,255,255,0.5);padding:16px;">加载精选阵容中</div>'; let list={items:[]}; try{ list=await _featuredLoad()||{items:[]}; }catch(e){} if(!list.items||!list.items.length){ p.innerHTML='<div style="color:rgba(255,255,255,0.4);padding:20px;text-align:center;">还没有精选阵容。'+(_isAdmin()?'点上方添加精选从项目里勾选添加。':'等待管理员添加。')+'</div>'; return; } const projs=[]; for(let i=0;i<list.items.length;i++){ const it=list.items[i]; try{ const pr=await fetch('projects/'+it.file); if(!pr.ok) continue; const pj=await pr.json(); projs.push({file:it.file,pj:pj}); }catch(e){} } projs.forEach(function(o){ try{ _featuredSync(o.file,o.pj); }catch(e){} }); const groups={}; projs.forEach(function(o){ const cat=(o.pj.category||'其他'); (groups[cat]=groups[cat]||[]).push(o); }); let h=''; Object.keys(groups).forEach(function(cat){ h+='<div style="margin:10px 0 4px;color:#4ecdc4;font-weight:800;font-size:0.92rem;">'+_esc(cat)+'</div>'; groups[cat].forEach(function(o){ h+=_featuredCard(o.file,o.pj); }); }); if(!h) h='<div style="color:rgba(255,255,255,0.4);padding:20px;">（项目数据加载失败）</div>'; p.innerHTML=h; _applySlots(p); _refreshDrSums(p); }
    window._featuredAddDlg = async function(){ const old=document.getElementById('featAddDlg'); if(old) old.remove(); let idx=[]; try{ const r=await fetch('projects/index.json'); if(r.ok) idx=await r.json(); }catch(e){ idx=[]; } if(!Array.isArray(idx)) idx=[]; const cur=await _featuredLoad(); const curSet={}; (cur.items||[]).forEach(function(x){ curSet[x.file]=1; }); const m=document.createElement('div'); m.id='featAddDlg'; m.style.cssText='position:fixed;inset:0;z-index:100004;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;'; const cats={}; idx.forEach(function(x){ const c=x.category||'其他'; (cats[c]=cats[c]||[]).push(x); }); let body=''; Object.keys(cats).forEach(function(c){ body+='<div style="color:#4ecdc4;font-weight:700;margin:6px 0 2px;">'+_esc(c)+'</div>'; cats[c].forEach(function(x){ body+='<label style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:0.82rem;color:rgba(255,255,255,0.85);cursor:pointer;"><input type="checkbox" data-file="'+_esc(x.file)+'" style="accent-color:#9c27b0;cursor:pointer;">'+_esc(x.name||x.file)+'</label>'; }); }); const box=document.createElement('div'); box.style.cssText='width:min(460px,92vw);max-height:86vh;overflow:auto;background:linear-gradient(160deg,#141a33,#0d1b2a);border:1px solid rgba(156,39,176,0.5);border-radius:12px;padding:14px;'; box.innerHTML='<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;"><b style="color:#ce93d8;font-size:0.95rem;">添加精选阵容</b><span style="flex:1;"></span><button id="featAddClose" style="background:transparent;border:none;color:#fff;font-size:1.2rem;cursor:pointer;">x</button></div><div style="color:rgba(255,255,255,0.5);font-size:0.7rem;margin-bottom:8px;">勾选要展示的项目（按 category 分区显示，添加后全员可见）</div>'+body+'<div style="display:flex;gap:8px;margin-top:12px;"><button id="featAddSave" style="flex:1;padding:9px;border:none;border-radius:8px;background:linear-gradient(135deg,#9c27b0,#7b1fa2);color:#fff;cursor:pointer;font-size:0.85rem;font-weight:700;">保存</button><button id="featAddCancel" style="padding:9px 14px;border:none;border-radius:8px;background:#666;color:#fff;cursor:pointer;font-size:0.85rem;">取消</button></div>'; m.appendChild(box); document.body.appendChild(m); m.querySelector('#featAddClose').addEventListener('click',function(){m.remove();}); m.querySelector('#featAddCancel').addEventListener('click',function(){m.remove();}); m.addEventListener('click',function(e){ if(e.target===m) m.remove(); }); m.querySelector('#featAddSave').addEventListener('click',async function(){ const files=Array.from(box.querySelectorAll('input[data-file]')).filter(function(c){return c.checked;}).map(function(c){return c.getAttribute('data-file');}); const items=files.map(function(f){return {file:f};}); const ok=await _featuredSave(items); if(ok){ try{ if(typeof showToast==='function') showToast('已保存精选阵容（全员可见）','success'); }catch(e){} m.remove(); _llTab('featured'); } else { try{ if(typeof showToast==='function') showToast('保存失败：需配置 Gist Token 且为管理员','error'); }catch(e){} } }); }
