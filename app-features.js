        ...
        function showForceUpdateOverlay(data, cur) {
            ...
            (async () => {
                ...
                const list = [];
                // 🔴 只给「一个」APP 安装包下载地址（可复制 / 可浏览器打开），绝不给主站·网页版入口：
                //    被门禁拦的用户一旦看到网页版链接就会直接用 web 版，不再装 APP（桌面端用户流失）。
                //    仅当自动解析下载地址失败时，才由 renderLinks 兜底显示 Gitee 发布页（同样是下载页）。
                if (giteeUrl) list.push({ label: 'Gitee 安装包直连（可 📋 复制链接，或点 🌐 用浏览器打开）', url: giteeUrl });
        ...
        function buildHiddenSwitchPart(ctx) {
            ...
            if (needFengHuangSwitch) {
                if (needXiaoYeSwitch) {
                    // 情况1：小野是切换进来的，02:31已下小野并接了精灵；🔴 2026-09-09 凤凰改为三段（02:44/02:49/02:50，先3级后满）
                    output += `02:44,下${firstCardName},上凤凰3级,\n`;
                    output += `02:49,上凤凰满,\n`;
                    output += `02:50,下凤凰,上${firstCardName}满${jlStr},\n`;
                } else if (xiaoYeDeployed) {
                    // 情况4：小野本来在阵容，用小野上下凤凰，最后切回小野接精灵（同样改三段）
                    output += `02:44,下小野,上凤凰3级,\n`;
                    output += `02:49,上凤凰满,\n`;
