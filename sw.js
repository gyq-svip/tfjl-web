...
const CACHE_VERSION = 's1.0.728';
const DEPLOY_TAG = 's20260905-1630';  // 部署时由 deploy.yml python 脚本注入为 's20260824-HHMM'（北京时区），SW_VERSION 消息携带到页面，根治「版本号日期消失」)
        ...
        Promise.resolve().then(() => {
            return self.clients.matchAll({ includeUncontrolled: true }).then(clients => {
                clients.forEach(client => {
                    ...
                    // 🔴 2026-08-30 修复：不再无条件发 NEW_VERSION_READY，否则每次 SW 更新检查（即使同版本）
                ...
                _timeoutFetch(request).then(async (networkResponse) => {
                    if (networkResponse && networkResponse.status === 200) {
                        ...
                        if (cachedText !== networkText) {
                            ...
                            // 🔴 2026-08-30 修复：图片/字体更新不再发 NEW_VERSION_READY，
                            //   避免皮肤图片更新触发版本气泡反复弹出。只有 HTML/JS/CSS（networkFirst）
